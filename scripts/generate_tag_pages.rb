...
# Place generated tag pages next to the site's source (sibling of _posts)
SITE_SOURCE = File.dirname(POSTS_DIR)
# Use a non-underscored directory so Jekyll will publish the files
TAG_DIR = File.join(SITE_SOURCE, 'tag')

# Collect tags from post front matter
tags = {}
...
def extract_tags_from_yaml_text(yaml_text)
  found = []
  # inline array: tags: [a, b]
  if yaml_text =~ /^\s*tags:\s*\[(.*?)\]/m
    inner = $1
...
if tags.empty?
  Dir.glob(File.join(POSTS_DIR, '*')) do |post|
    ...
    if content =~ /\A---\s*(.*?)\s*---/m
      yaml_text = $1
      extracted = extract_tags_from_yaml_text(yaml_text)
      Array(extracted).each do |tag|
        tags[tag] ||= 0
        tags[tag] += 1
      end
    end
...
tags.each do |tag, count|
  filepath = File.join(TAG_DIR, "#{tag}.md")
  if File.exist?(filepath)
    ...
    File.open(filepath, 'w', encoding: 'utf-8') do |f|
      f.puts "---"
      f.puts "layout: tag"
      f.puts "title: \"#{tag.to_s.gsub('"', '\\"')}\""
      f.puts "tag: \"#{tag.to_s.gsub('"', '\\"')}\""
...
puts "Generated #{tags.size} tag pages in #{TAG_DIR}. Permalinks are under /tag/<tag>/."
